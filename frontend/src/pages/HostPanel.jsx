  ...
  return (
    <div className="container">
      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1.5rem' }}>
        ...
        <h2 style={{ margin: 0, fontWeight: 700, color: '#111' }}>Create Contest</h2>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', alignItems: 'start', gap: '1.5rem' }}>

        <div>
          <div className="glass-panel">
            <h3 style={{ borderBottom: '1px solid var(--border-color)', paddingBottom: '0.5rem', marginBottom: '1rem', color: '#111', fontWeight: 700 }}>Contest Configuration</h3>
            <p style={{ color: '#999', fontSize: '0.85rem', marginBottom: '1rem', lineHeight: 1.6 }}>
            ...
            <div style={{ padding: '0.75rem', background: '#fafafa', borderLeft: '3px solid #000', borderRadius: '4px', marginBottom: '1rem' }}>
              <p style={{ color: '#111', fontWeight: 600, margin: '0 0 0.25rem' }}>{MODE_INFO[mode].label}</p>
              <p style={{ color: '#888', fontSize: '0.85rem', margin: 0 }}>{MODE_INFO[mode].desc}</p>
            </div>
            <div className="form-group">
        ...
        <div>
          <div className="glass-panel">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--border-color)', paddingBottom: '0.75rem', marginBottom: '1rem' }}>
              <h3 style={{ margin: 0, color: '#111', fontWeight: 700 }}>Problems ({contestQuestions.length})</h3>
            ...
            {contestQuestions.map((q, idx) => (
              <div key={idx} style={{ background: '#fafafa', border: '1px solid #f0f0f0', padding: '1rem', marginBottom: '0.75rem', borderRadius: '8px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
                  <div>
                    <div style={{ fontWeight: 600, color: '#111', marginBottom: '0.25rem' }}>{idx + 1}. {q.title}</div>
      ...
      {showBankModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.25)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '1rem' }}>
          <div style={{ background: '#fff', border: '1px solid var(--border-color)', borderRadius: '10px', padding: '1.5rem', width: '100%', maxWidth: '600px', maxHeight: '80vh', display: 'flex', flexDirection: 'column', boxShadow: '0 20px 60px rgba(0,0,0,0.15)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem', borderBottom: '1px solid var(--border-color)', paddingBottom: '0.5rem' }}>
              <h3 style={{ margin: 0, color: '#111', fontWeight: 700 }}>Problem Bank</h3>
            ...
            <div style={{ overflowY: 'auto', flex: 1 }}>
              {bankProblems.length === 0 ? (
                <p style={{ color: '#bbb', textAlign: 'center', padding: '2rem' }}>No problems available. Go to Problems UI to add some.</p>
              ) : bankProblems.map(p => {
                const alreadyAdded = contestQuestions.find(q => q.bankId === p.id);
                const isSelected = selectedBankIds.has(p.id);
                return (
                  <div key={p.id} onClick={() => { if(!alreadyAdded) toggleBankSelect(p.id); }} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.75rem', borderBottom: '1px solid #f5f5f5', background: isSelected ? '#f5f5f5' : 'transparent', borderRadius: '4px', transition: 'background 0.15s', cursor: alreadyAdded ? 'default' : 'pointer' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', flex: 1, minWidth: 0 }}>
                      {!alreadyAdded && (
                        <input
                          ...
                          style={{ width: '18px', height: '18px', cursor: 'pointer', accentColor: '#000', flexShrink: 0 }}
                        />
                      )}
                      <div style={{ minWidth: 0 }}>
                        <div style={{ fontWeight: 600, color: '#111' }}>{p.title}</div>
                        <div style={{ fontSize: '0.8rem', color: '#999', marginTop: '0.15rem', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.description?.slice(0, 80)}...</div>
                      </div>
                    </div>
                    {alreadyAdded ? (
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.25)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '1rem' }}>
          <div style={{ background: '#fff', border: '1px solid var(--border-color)', borderRadius: '10px', padding: '1.5rem', width: '100%', maxWidth: '1200px', maxHeight: '90vh', display: 'flex', gap: '2rem', boxShadow: '0 20px 60px rgba(0,0,0,0.15)' }}>
            <div style={{ flex: 1, overflowY: 'auto', paddingRight: '0.5rem' }}>
              <h3 style={{ marginBottom: '1rem', borderBottom: '1px solid var(--border-color)', paddingBottom: '0.5rem', color: '#111', fontWeight: 700 }}>{editingIdx !== null ? 'Edit Problem' : 'New Problem'}</h3>
              <form onSubmit={saveProblem}>
                ...
                <div className="form-group" style={{ marginBottom: '1.5rem' }}>
                  <label style={{ fontSize: '1rem', color: '#111', fontWeight: 600 }}>Problem Statement</label>
                ...
                <div style={{ marginBottom: '0.5rem' }}>
                  ...
                  {draft.test_cases.map((tc, i) => (
                    <div key={i} style={{ display: 'flex', gap: '0.5rem', marginBottom: '0.5rem', alignItems: 'flex-start' }}>
                      <span style={{ color: '#bbb', fontSize: '0.8rem', minWidth: '20px', marginTop: '0.5rem' }}>{i + 1}.</span>
