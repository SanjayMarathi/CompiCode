...
export default function HostPanel() {
  ...
  const openBank = async () => {
    try { const res = await axios.get(`${API_URL}/questions`); setBankProblems(res.data); } catch {}
  ...
  const deployContest = async () => {
    ...
    try {
      ...
      const res = await axios.post(`${API_URL}/contests`, {
        ...
        evaluation_mode: evaluationMode, scheduled_start_time: scheduledStart ? scheduledStart : null,
        selected_questions: finalSelection
      });
      navigate(`/contest/${res.data.link_code}`);
    } catch (err) { alert(err.response?.data?.detail || 'Failed to create contest. Is the server running?'); }
  };

  const MODE_INFO = {
    standard: { label: 'Standard', desc: 'Players solve all problems independently. Ranked by total score, then by lowest penalty.' },
    sudden_death: { label: 'Sudden Death', desc: 'Everyone solves the same problem simultaneously. First to pass advances the lobby to the next round.' },
    timed: { label: 'Timed Mode', desc: 'Each problem has its own countdown. Once time runs out on a problem, it locks for that player.' },
  };

  const editorTheme = document.documentElement.getAttribute('data-theme') === 'dark' ? 'vs-dark' : 'vs-dark';
  ...
  return (
    <div className="container">
      <div className="fade-in" style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1.5rem' }}>
        ...
        <h2 style={{ margin: 0, fontWeight: 700 }}>Create Contest</h2>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', alignItems: 'start', gap: '1.5rem' }}>

        <div className="glass-panel fade-in-up stagger-1">
          <h3 style={{ borderBottom: '1px solid var(--border-color)', paddingBottom: '0.5rem', marginBottom: '1rem', fontWeight: 700 }}>Contest Configuration</h3>
          <p style={{ color: 'var(--text-secondary)', fontSize: '0.85rem', marginBottom: '1rem', lineHeight: 1.6 }}>
            Configure the rules and environment for your upcoming contest.
          </p>
          <div className="form-group"><label>Contest Title</label><input className="form-input" required placeholder="e.g. Weekly Round #12" value={title} onChange={e => setTitle(e.target.value)} /></div>
          <div className="form-group"><label>Description (optional)</label><textarea className="form-input" rows="2" placeholder="Brief description..." value={desc} onChange={e => setDesc(e.target.value)} /></div>
          <div className="form-group"><label>Mode</label><select className="form-input" value={mode} onChange={e => setMode(e.target.value)}><option value="standard">Standard</option><option value="sudden_death">Sudden Death</option><option value="timed">Timed Mode</option></select></div>
          <div className="form-group"><label>Visibility</label><select className="form-input" value={visibility} onChange={e => setVisibility(e.target.value)}><option value="public">Public (Anyone with link can join)</option><option value="private">Private (Host must approve join requests)</option></select></div>
          <div style={{ padding: '0.75rem', background: 'var(--bg-secondary)', borderLeft: '3px solid var(--primary)', borderRadius: '6px', marginBottom: '1rem', transition: 'all 0.3s' }}>
            <p style={{ color: 'var(--text-primary)', fontWeight: 600, margin: '0 0 0.25rem' }}>{MODE_INFO[mode].label}</p>
            <p style={{ color: 'var(--text-secondary)', fontSize: '0.85rem', margin: 0 }}>{MODE_INFO[mode].desc}</p>
          </div>
          <div className="form-group"><label>Overall Time Limit (minutes)</label><input type="number" min="1" max="480" className="form-input" value={overallLimit} onChange={e => setOverallLimit(e.target.value)} /></div>
          <div className="form-group"><label>Wrong Answer Penalty (points)</label><input type="number" min="0" className="form-input" value={penalty} onChange={e => setPenalty(e.target.value)} /></div>
          <div className="form-group"><label>Scheduled Start Time (Optional)</label><input type="datetime-local" className="form-input" value={scheduledStart} onChange={e => setScheduledStart(e.target.value)} /></div>
        ...
        <div className="glass-panel fade-in-up stagger-2">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--border-color)', paddingBottom: '0.75rem', marginBottom: '1rem' }}>
            <h3 style={{ margin: 0, fontWeight: 700 }}>Problems ({contestQuestions.length})</h3>
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              <button className="btn btn-secondary" onClick={openBank} style={{ padding: '0.4rem 1rem', fontSize: '0.85rem' }}>Problems</button>
              <button className="btn btn-primary" onClick={openNew} style={{ padding: '0.4rem 1rem', fontSize: '0.85rem' }}>+ New</button>
            </div>
          </div>
          {contestQuestions.length === 0 && (
            <div style={{ textAlign: 'center', padding: '2rem', color: 'var(--text-tertiary)' }}>
              <p>No problems added yet.</p>
              <p style={{ fontSize: '0.85rem' }}>Click "+ New" to create your first problem.</p>
            </div>
          )}
          {contestQuestions.map((q, idx) => (
            <div key={idx} className="fade-in" style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border-color)', padding: '1rem', marginBottom: '0.75rem', borderRadius: '10px', transition: 'all 0.25s', animationDelay: `${idx * 0.05}s` }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
                <div>
                  <div style={{ fontWeight: 600, color: 'var(--text-primary)', marginBottom: '0.25rem' }}>{idx + 1}. {q.title}</div>
                  <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>{q.points} pts &nbsp;|&nbsp; {q.test_cases.length} test cases{mode === 'timed' && ` | ${q.time_limit}s limit`}</div>
                </div>
      ...
      {showBankModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'var(--overlay-bg)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '1rem' }}>
          <div className="fade-in-scale" style={{ background: 'var(--modal-bg)', border: '1px solid var(--border-color)', borderRadius: '14px', padding: '1.5rem', width: '100%', maxWidth: '600px', maxHeight: '80vh', display: 'flex', flexDirection: 'column', boxShadow: 'var(--shadow-lg)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem', borderBottom: '1px solid var(--border-color)', paddingBottom: '0.5rem' }}>
              <h3 style={{ margin: 0, fontWeight: 700 }}>Problem Bank</h3>
                ...
                {bankProblems.filter(p => !contestQuestions.find(q => q.bankId === p.id)).length > 0 && (
                  <>
                    ...
                    {selectedBankIds.size > 0 && (
                      <button className="btn btn-primary" onClick={addSelectedFromBank} style={{ padding: '0.3rem 0.8rem', fontSize: '0.8rem' }}>Add Selected ({selectedBankIds.size})</button>
                    )}
                  </>
                )}
              </div>
            ...
            <div style={{ overflowY: 'auto', flex: 1 }}>
              {bankProblems.length === 0 ? (
                <p style={{ color: 'var(--text-tertiary)', textAlign: 'center', padding: '2rem' }}>No problems available.</p>
              ) : bankProblems.map(p => {
                const alreadyAdded = contestQuestions.find(q => q.bankId === p.id);
                const isSelected = selectedBankIds.has(p.id);
                return (
                  <div key={p.id} onClick={() => { if(!alreadyAdded) toggleBankSelect(p.id); }} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.75rem', borderBottom: '1px solid var(--border-color)', background: isSelected ? 'var(--primary-subtle)' : 'transparent', borderRadius: '6px', transition: 'all 0.2s', cursor: alreadyAdded ? 'default' : 'pointer' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', flex: 1, minWidth: 0 }}>
                      {!alreadyAdded && (<input type="checkbox" checked={isSelected} onChange={() => toggleBankSelect(p.id)} style={{ width: '18px', height: '18px', cursor: 'pointer', accentColor: 'var(--primary)', flexShrink: 0 }} />)}
      ...
      {showProblemForm && (
        <div style={{ position: 'fixed', inset: 0, background: 'var(--overlay-bg)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '1rem' }}>
          <div className="fade-in-scale" style={{ background: 'var(--modal-bg)', border: '1px solid var(--border-color)', borderRadius: '14px', padding: '1.5rem', width: '100%', maxWidth: '1200px', maxHeight: '90vh', display: 'flex', gap: '2rem', boxShadow: 'var(--shadow-lg)' }}>
            <div style={{ flex: 1, overflowY: 'auto', paddingRight: '0.5rem' }}>
              <h3 style={{ marginBottom: '1rem', borderBottom: '1px solid var(--border-color)', paddingBottom: '0.5rem', fontWeight: 700 }}>{editingIdx !== null ? 'Edit Problem' : 'New Problem'}</h3>
              <form onSubmit={saveProblem}>
                <div className="form-group" style={{ marginBottom: '1.5rem' }}><label style={{ fontSize: '1rem', color: 'var(--text-primary)', fontWeight: 600 }}>Problem Title</label><input required className="form-input" placeholder="e.g. Two Sum" value={draft.title} onChange={e => setDraft({ ...draft, title: e.target.value })} style={{ padding: '0.8rem', fontSize: '1rem' }} /></div>
                <div className="form-group" style={{ marginBottom: '1.5rem' }}><label style={{ fontSize: '1rem', color: 'var(--text-primary)', fontWeight: 600 }}>Problem Statement</label><textarea required className="form-input" rows="5" placeholder="Describe the problem..." value={draft.description} onChange={e => setDraft({ ...draft, description: e.target.value })} style={{ padding: '0.8rem', fontFamily: 'inherit', fontSize: '0.95rem' }} /></div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', marginBottom: '1.5rem' }}>
                  ...
                  {mode === 'timed' && (<div className="form-group"><label>Time Limit (seconds)</label><input type="number" min="30" className="form-input" value={draft.time_limit} onChange={e => setDraft({ ...draft, time_limit: parseInt(e.target.value) })} /></div>)}
                </div>
                <div style={{ marginBottom: '0.5rem' }}>
                  ...
                  {draft.test_cases.map((tc, i) => (
                    <div key={i} style={{ display: 'flex', gap: '0.5rem', marginBottom: '0.5rem', alignItems: 'flex-start' }}>
                      <span style={{ color: 'var(--text-tertiary)', fontSize: '0.8rem', minWidth: '20px', marginTop: '0.5rem' }}>{i + 1}.</span>
                      <textarea className="form-input" rows="2" placeholder="Input" value={tc.input_data} onChange={e => updateTC(i, 'input_data', e.target.value)} style={{ flex: 1, fontFamily: 'monospace', resize: 'vertical' }} />
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', borderLeft: '1px solid var(--border-color)', paddingLeft: '2rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem', borderBottom: '1px solid var(--border-color)', paddingBottom: '0.5rem' }}>
                <h3 style={{ margin: 0, fontWeight: 700 }}>Sandbox Testing</h3>
              ...
              <div style={{ flex: 1, border: '1px solid var(--border-color)', borderRadius: '8px', overflow: 'hidden', minHeight: '300px' }}>
                <Editor height="100%" theme="vs-dark" language={sandboxLang === 'cpp' ? 'cpp' : sandboxLang} value={sandboxCode} onChange={val => setSandboxCode(val)} options={{ minimap: { enabled: false }, fontSize: 14 }} />
              </div>
              <div style={{ marginTop: '1rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <button type="button" className="btn btn-primary" onClick={runSandbox} disabled={sandboxTesting}>{sandboxTesting ? 'Running...' : 'Run Tests'}</button>
              </div>
              {sandboxResults && (
                <div style={{ marginTop: '1rem', background: 'var(--bg-secondary)', padding: '1rem', borderRadius: '8px', maxHeight: '200px', overflowY: 'auto', border: '1px solid var(--border-color)' }}>
                  {sandboxResults.map((res, i) => {
                     let sanitizedActual = res.actual || 'No output';
                     if (sanitizedActual.includes('File "') && sanitizedActual.includes('line')) { const lines = sanitizedActual.split('\n').map(l => l.trim()).filter(l => l); sanitizedActual = lines[lines.length - 1] || 'Runtime/Syntax Error'; }
                     return (
