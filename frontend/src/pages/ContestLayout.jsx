...
export default function ContestLayout({ userObj }) {
  ...
  if (!contest) return (
    <div className="container" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '60vh' }}>
      ...
      <h2 className="pulse-text" style={{ color: '#333' }}>Entering Arena...</h2>
    </div>
  );
  ...
  if (!isHost && participantStatus === 'pending') {
    return (
      <div className="container" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '60vh' }}>
        <div className="loader" style={{ marginBottom: '1rem', width: '50px', height: '50px', border: '5px solid #eee', borderTop: '5px solid #000', borderRadius: '50%', animation: 'spin 1s linear infinite' }}></div>
        <h2 className="pulse-text" style={{ color: '#333', marginBottom: '0.5rem' }}>Waiting for Approval...</h2>
        <p style={{ color: '#999' }}>This is a private contest. Please wait for the host to accept your join request.</p>
      </div>
    );
  }
  ...
  if (!isHost && participantStatus === 'rejected') {
    return (
      <div className="container" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '60vh' }}>
        ...
        <p style={{ color: '#999' }}>Your request to join this contest was rejected by the host.</p>
  ...
  if (contest.status === 'ended') {
    return (
      <div className="container">
        <div style={{ textAlign: 'center', padding: '4rem', background: '#fafafa', borderRadius: '10px', border: '1px solid var(--border-color)', marginTop: '2rem', marginBottom: '2rem' }}>
          <h2 style={{ color: '#333', fontSize: '2.5rem', marginBottom: '1rem', fontWeight: 800 }}>Contest Ended</h2>
          <p style={{ color: '#999', fontSize: '1.1rem', marginBottom: '2rem' }}>This contest has been terminated by the host. The final standings are locked.</p>
        ...
        <div className="glass-panel" style={{ width: '100%' }}>
          <h3 style={{ marginBottom: '1rem', color: '#111', fontWeight: 700 }}>Final Standings</h3>
    ...
    return (
      <div className="container">
        <div className="glass-panel" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
          <div>
            <h1 style={{ margin: 0, color: '#000' }}>{contest.title} <span className="badge badge-yellow" style={{ marginLeft: '1rem' }}>{meta.label}</span></h1>
            {contest.host_name && (
              <div style={{ marginTop: '0.4rem', color: '#999', fontSize: '0.9rem', display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
                Hosted by <strong style={{ color: '#111' }}>{contest.host_name}</strong>
                <span style={{ color: 'var(--success)', fontSize: '1rem' }}>✓</span>
              </div>
            )}
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginTop: '0.5rem', flexWrap: 'wrap' }}>
              <p style={{ color: '#999', margin: 0 }}>
                Access Code: <strong style={{ color: '#000', cursor: 'pointer', background: '#f5f5f5', padding: '0.2rem 0.5rem', borderRadius: '4px', letterSpacing: '2px', border: '1px solid #eee' }} onClick={() => navigator.clipboard.writeText(linkCode)} title="Click to copy">{linkCode}</strong>
              </p>
              <button className="btn btn-primary" onClick={copyInviteLink} style={{ padding: '0.3rem 0.8rem', fontSize: '0.8rem', background: linkCopied ? 'var(--success)' : '#000', transition: 'background 0.2s' }}>
                {linkCopied ? '✓ Link Copied!' : '📋 Copy Invite Link'}
              </button>
            </div>
          </div>
          {isHost && <span style={{ fontSize: '0.8rem', background: '#f5f5f5', color: '#000', border: '1px solid #ddd', padding: '0.25rem 0.75rem', borderRadius: '4px', fontWeight: 600 }}>HOST</span>}
        </div>

        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          <div className="glass-panel" style={{ padding: '2.5rem 2rem' }}>
            <div style={{ borderLeft: '4px solid #000', paddingLeft: '1rem', marginBottom: '2rem' }}>
              <h2 style={{ color: '#000', margin: '0 0 0.5rem', fontWeight: 700 }}>{meta.label}</h2>
              <p style={{ color: '#888', margin: 0 }}>{meta.desc}</p>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '1rem', marginBottom: '2rem' }}>
              <div style={{ background: '#fafafa', padding: '1rem', borderRadius: '8px', textAlign: 'center', border: '1px solid #f0f0f0' }}>
                <div style={{ fontSize: '1.5rem', fontWeight: 700, color: '#000' }}>{contest.questions.length}</div>
                <div style={{ fontSize: '0.8rem', color: '#999', textTransform: 'uppercase', letterSpacing: '0.05em', marginTop: '0.25rem' }}>Problems</div>
              </div>
              <div style={{ background: '#fafafa', padding: '1rem', borderRadius: '8px', textAlign: 'center', border: '1px solid #f0f0f0' }}>
                <div style={{ fontSize: '1.5rem', fontWeight: 700, color: '#000' }}>{contest.overall_time_limit}m</div>
                <div style={{ fontSize: '0.8rem', color: '#999', textTransform: 'uppercase', letterSpacing: '0.05em', marginTop: '0.25rem' }}>Time Limit</div>
              </div>
              <div style={{ background: '#fafafa', padding: '1rem', borderRadius: '8px', textAlign: 'center', border: '1px solid #f0f0f0' }}>
                <div style={{ fontSize: '1.5rem', fontWeight: 700, color: 'var(--danger)' }}>{formatPenalty(contest.penalty_per_wrong_answer)}</div>
                <div style={{ fontSize: '0.8rem', color: '#999', textTransform: 'uppercase', letterSpacing: '0.05em', marginTop: '0.25rem' }}>WA Penalty</div>
              </div>
            </div>

            {isHost ? (
              <div style={{ textAlign: 'center' }}>
                <p style={{ color: '#888', marginBottom: '1.5rem' }}>Share the invite link with participants. {contest.scheduled_start_time ? 'The contest will start automatically on time.' : 'Start the contest when everyone is ready.'}</p>
                
                {contest.scheduled_start_time ? (
                  <>
                    <div style={{ fontSize: '3rem', fontWeight: 700, color: '#000', marginBottom: '1rem', fontFamily: 'Consolas, monospace' }}>
                      {scheduledCountdown !== null && scheduledCountdown > 0 ? formatTime(scheduledCountdown) : '0:00'}
                    </div>
                    <h3 className="pulse-text" style={{ color: '#555', marginBottom: '0.5rem' }}>
                      {scheduledCountdown !== null && scheduledCountdown <= 0 ? 'Starting automatically...' : 'Host: Contest starts automatically...'}
                    </h3>
                  </>
                ) : (
                  <button className="btn btn-primary" onClick={handleHostStart} style={{ padding: '0.9rem 3rem', fontSize: '1.1rem' }}>Start Contest →</button>
                )}

                {contest.visibility === 'private' && (
                  <div style={{ marginTop: '2rem', borderTop: '1px solid var(--border-color)', paddingTop: '1.5rem', textAlign: 'left' }}>
                    <h3 style={{ color: '#111', marginBottom: '1rem', fontWeight: 700 }}>Pending Participants ({pendingParticipants.length})</h3>
                    {pendingParticipants.length === 0 ? (
                      <p style={{ color: '#bbb' }}>No pending requests.</p>
                    ) : (
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                        {pendingParticipants.map(p => (
                          <div key={p.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#fafafa', padding: '0.75rem 1rem', borderRadius: '6px', border: '1px solid #f0f0f0' }}>
                            <strong style={{ color: '#111' }}>{p.username}</strong>
              ...
              <div style={{ textAlign: 'center' }}>
                <p style={{ color: '#888', marginBottom: '1.5rem' }}>The arena is ready. Join the lobby to secure your spot.</p>
              ...
              <div style={{ textAlign: 'center' }}>
                <div className="loader" style={{ margin: '0 auto 1.5rem', width: '30px', height: '30px', border: '3px solid #eee', borderTop: '3px solid #000', borderRadius: '50%', animation: 'spin 1s linear infinite' }}></div>
                <h3 className="pulse-text" style={{ color: '#555', marginBottom: '0.5rem' }}>Waiting for host to start the contest...</h3>
                <p style={{ color: '#bbb', fontSize: '0.9rem' }}>The arena will automatically open once the host begins.</p>
              </div>
            )}
          </div>

          <div className="glass-panel" style={{ width: '100%' }}>
            <h3 style={{ marginBottom: '1rem', color: '#111', fontWeight: 700 }}>Standings</h3>
            <CodeforcesStandings leaderboard={leaderboard} questions={contest.questions} evaluationMode={contest.evaluation_mode} isHost={isHost} onKick={handleKick} mode={contest.mode} />
          </div>
        </div>
      </div>
  ...
  return (
    <div className="container">
        ...
        <div>
          ...
          {contestStarted && (contest.mode === 'standard' || contest.mode === 'timed') && (
            <div style={{ marginTop: '0.75rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              ...
              <span style={{ fontSize: '1.2rem', fontWeight: 600, color: '#000', fontFamily: 'Consolas, monospace' }}>
                {formatTime(Math.max(0, contest.overall_time_limit * 60 - elapsedSeconds))}
              </span>
            </div>
          )}
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginTop: '0.5rem', flexWrap: 'wrap' }}>
            <p style={{ color: '#999', margin: 0 }}>Access Code: <strong style={{ color: '#000', cursor: 'pointer', background: '#f5f5f5', padding: '0.2rem 0.5rem', borderRadius: '4px', border: '1px solid #eee' }} onClick={() => navigator.clipboard.writeText(linkCode)}>{linkCode}</strong></p>
            <button className="btn btn-primary" onClick={copyInviteLink} style={{ padding: '0.25rem 0.7rem', fontSize: '0.78rem', background: linkCopied ? 'var(--success)' : '#000', transition: 'background 0.2s' }}>
      ...
      {isLateJoiner && contest.mode !== 'sudden_death' && (
        <div style={{ padding: '0.75rem 1.25rem', background: '#fafafa', border: '1px solid #eee', borderRadius: '6px', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          <span style={{ fontSize: '1.2rem' }}>⚡</span>
          <p style={{ color: '#555', margin: 0, fontSize: '0.9rem' }}>Contest is already in progress. You can still participate — pick a problem and start solving!</p>
        </div>
      )}

      {/* Enter Arena button for active Sudden Death contests */}
      {contestStarted && contest.mode === 'sudden_death' && (
        <div style={{ textAlign: 'center', padding: '3rem', background: '#fafafa', border: '1px solid #eee', borderRadius: '10px', marginBottom: '2rem' }}>
          <h2 style={{ color: '#000', marginBottom: '1rem', fontWeight: 700 }}>Sudden Death is Active!</h2>
          <p style={{ color: '#888', marginBottom: '2rem' }}>The match is currently ongoing. Jump into the arena to participate.</p>
                ...
                <div key={q.id} className="glass-panel" style={{ marginBottom: '1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderLeft: isSolved ? '4px solid var(--success)' : '4px solid transparent' }}>
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                      <h3 style={{ margin: 0, color: '#111' }}>{q.title}</h3>
                      {isSolved && <span style={{ background: 'var(--success)', color: '#fff', fontSize: '0.7rem', fontWeight: 700, padding: '0.15rem 0.5rem', borderRadius: '4px', letterSpacing: '1px' }}>✓ SOLVED</span>}
                    </div>
                    <p style={{ color: '#999', marginTop: '0.5rem', fontSize: '0.9rem' }}>Points: <span style={{ color: isSolved ? 'var(--success)' : '#000', fontWeight: 600 }}>{q.points}</span> {contest.mode === 'timed' && `| Time Limit: ${q.time_limit}s`}</p>
                  </div>
                  {isSolved ? (
                    <Link to={`/solve/${contest.id}/${q.id}`} style={{ padding: '0.5rem 1.5rem', background: '#f0fdf4', border: '1px solid var(--success)', borderRadius: '6px', color: 'var(--success)', fontSize: '0.9rem', fontWeight: 600, textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: '0.5rem', transition: 'background 0.15s' }} onMouseEnter={e => e.currentTarget.style.background='#dcfce7'} onMouseLeave={e => e.currentTarget.style.background='#f0fdf4'}>
                      ✓ Solved — Review
                    </Link>
                  ) : (
                    (() => {
